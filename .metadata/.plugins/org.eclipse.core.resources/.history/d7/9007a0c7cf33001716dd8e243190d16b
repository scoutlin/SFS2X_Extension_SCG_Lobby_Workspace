...
SFSEventParam.JOINED_ROOMS: the list of Rooms previously joined by the User ( List or Room )
SFSEventParam.PLAYER_IDS_BY_ROOM: a map of PlayerId by Room, for each Room joined by the User ( Map of Room, Integer)
SFSEventParam.DISCONNECTION_REASON: the reason of the disconnection (where it is possible to detect it) ( ClientDisconnectionReason )
 */
